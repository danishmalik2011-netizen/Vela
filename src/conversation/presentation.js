(() => {
  ...
  function createAssistantArticle(document, options) {
    ...
    const initialVerb = article.dataset.taskMode === "research" ? "Investigating…" : (article.dataset.taskMode === "code" ? "Architecting…" : (article.dataset.taskMode === "write" ? "Gathering ideas…" : "Thinking…"));
    const initialCursorHtml = globalThis.VelaStreamingSpinner?.createIndicatorHTML?.(initialVerb) ||
      `<span class="streaming-cursor streaming-indicator" role="status" aria-live="polite" aria-label="Generating response"><span class="streaming-indicator-spinner" aria-hidden="true"><svg class="streaming-spinner-svg" viewBox="0 0 16 16" width="14" height="14" fill="none"><circle class="spinner-track" cx="8" cy="8" r="6" stroke="currentColor" stroke-width="1.75" /><circle class="spinner-head" cx="8" cy="8" r="6" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-dasharray="11 27" /></svg><span class="streaming-spinner-core" aria-hidden="true"></span></span><span class="streaming-indicator-label"><span class="streaming-indicator-verb">${initialVerb}</span></span></span>`;
