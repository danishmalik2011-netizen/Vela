(() => {
  ...
  const STARTER_PROMPTS = Object.freeze([
    ["Help me turn a rough idea into a clear plan", "Turn a rough idea into a clear plan"],
    ["Explain a difficult topic in simple terms", "Explain a difficult topic simply"],
    ["Help me write and refine something", "Write and refine something"],
    ["Review my code and suggest improvements", "Review code and suggest improvements"],
    // First-run artifact demo: immediately shows the canvas opening with a
    // live preview so new users discover the workspace model.
    ["/build Create a small self-contained HTML landing page for a fictional ceramics studio — a hero section, three product cards, and a contact footer, with warm minimal styling.", "Build a small live artifact"]
  ]);
  ...
  function followupsFor(mode) {
    ...
    const buttons = set.map(([action, label, special]) =>
      `<button class="response-followup" type="button" data-followup="${action}"${special ? ` data-special="true"` : ""}>${label}</button>`).join("");
    return `${buttons}<button class="response-followup" type="button" data-followup="canvas" data-special="true">Canvas</button><button class="response-followup" type="button" data-followup="export" data-special="true">Export</button>`;
  }
  ...
  function createAssistantArticle(document, options) {
    ...
    article.dataset.taskMode = validModes.includes(taskMode) ? taskMode : "chat";
    if (timestamp) article.dataset.timestamp = String(timestamp);
    const followupButtons = followupsFor(article.dataset.taskMode);
    article.innerHTML = `<div class="message-body"><div class="thought sage-live-thought"><button class="thought-toggle" type="button" aria-expanded="false"><span class="thought-dot"></span><span class="sage-thought-label">Thinking…</span><svg class="icon thought-chevron" style="width:13px;height:13px" viewBox="0 0 24 24" aria-hidden="true"><path d="m7 10 5 5 5-5"></path></svg></button><div class="thought-summary" hidden></div><div class="thought-details"><div class="thought-details-inner"><div class="thought-details-copy markdown-body"></div></div></div></div><div class="sage-response markdown-body"><span class="streaming-cursor"></span></div><div class="response-actions"><button class="mini-action sage-copy-response" type="button" aria-label="Copy response" title="Copy response"><svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><rect x="9" y="9" width="11" height="11" rx="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></button><button class="mini-action sage-regenerate-response" type="button" aria-label="Regenerate response" title="Regenerate response"><svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M20 7v5h-5M4 17v-5h5"></path><path d="M6.1 9A7 7 0 0 1 18.7 6L20 8M4 16l1.3 2A7 7 0 0 0 18 15"></path></svg></button><span class="response-followups">${followupButtons}</span></div><time class="message-time assistant-message-time"></time></div>`;
    if (timestamp) {
  ...
  globalThis.VelaConversationPresentation = Object.freeze({ STARTER_PROMPTS, formatTime, reasoningSummary, createStartup, createUserArticle, createAssistantArticle });
})();
